...
import { useGetHouseByCodeQuery, useUpdateHouseMutation, useUploadPhotoHouseMutation } from '../../features/api/apiHousesSlice';
...
export default function HouseFormEdit() {
    ...
    const params = useParams(); // Instanciamos la vaiable de useParams
    ...
    const handleChangePhotoHouse = (e) => {
        ...
        try {
            ...
            if (response.data.status == "error") {
                ...
                if (file) {
                    ...
                    const code = response.data.updatedHome.code
                    uploadPhotoHouse({ code: code, file: formData })
                }
                Swal.fire({
                    position: "top-end",
                    icon: "success",
    ...
    const { data: house, isLoading, isError, error } = useGetHouseByCodeQuery(params.id); // Obtenemos los datos de la casa por el codigo
        ...
        <div className='hero flex min-h-screen items-center justify-center pt-20'>
            <div className='flex max-w-4xl flex-col items-center justify-center gap-6 pb-10'>
                ...
                <HouseForm props={{
                    ...
                    handleChangeAvatar: handleChangePhotoHouse,
                    house: house
                }} />
            </div>
        </div>
